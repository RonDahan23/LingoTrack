...
- **Translation walks a provider chain**, it is not one API. [backend/src/services/translation/providers.ts](backend/src/services/translation/providers.ts) tries Google dict (`translate_a/t?client=dict-chrome-ex`, on `clients5.google.com` then `translate.googleapis.com`) → Google mobile (`translate.google.com/m`, scraped HTML) → Google `gtx` (JSON) → MyMemory; [backend/src/services/translationService.ts](backend/src/services/translationService.ts) is the cache shell around it (`Translation` table, unique on `source+target`, keyed on the lower-cased/whitespace-collapsed source). `GET /api/translate?text=` serves word taps and per-line translation.
  - The chain exists because **every free key-less provider fails on a datacenter IP**, each differently: `gtx` 429s after a handful of calls per IP, MyMemory has a per-IP *daily* character quota. One provider alone was the cause of "Translation failed" in production while working fine from a laptop. Keep at least two live paths.
  - **Ordering is by availability, not output quality** (all the Google paths return identical text). `client=dict-chrome-ex` — the endpoint Chrome's own translate feature uses — leads because it is metered in a **different quota bucket**: it served 12 concurrent requests from an address where `gtx` *and* `translate.google.com/m` were both already returning 429, which is exactly the state that surfaced as "Translation service is busy" in the player. It is listed twice (two hosts, same endpoint) so losing one host is not losing the provider.
  - `parseGoogleDictResponse` requires **every** element to be a string before treating a body as the dict form. The same host also serves the `gtx` envelope, whose trailing element is the detected *language tag* — a looser check cached `"en"` as though it were the Hebrew translation. A gtx-shaped body is delegated to `parseGoogleResponse` instead.
  - `translate.google.com/m` sits behind it, still ahead of `gtx`. Its cost is that it is scraped — `parseGoogleMobileResponse` returns `''` on a markup change so the chain moves on instead of throwing.
  - **Lingva was tried and removed.** Every public instance (`lingva.ml` and mirrors) sits behind a Cloudflare bot challenge returning 403, which is exactly how a datacenter IP gets treated. Don't re-add a community proxy without checking it from outside a residential connection.
  - **A result carrying no Hebrew is treated as a provider failure, not a translation** (`looksUntranslated`). Google echoes back anything it cannot translate, so a bug upstream that asks for a non-word gets a confident non-answer. That is precisely how a lemmatizer bug shipped: `lemmatizeAdjective` stripped `-er` off any word ("conquer" → "conqu", "water" → "wat", "remember" → "rememb"), the translator echoed the non-word, and the echo was cached and then served as the **correct answer** in a Hebrew quiz. The check is "the target script is missing" rather than "output equals input", since a partial echo is just as wrong, and it is conditioned on the source containing Latin letters so that translating "1999" is not read as a failure. Applied on cache **read** as well as write, so rows poisoned before the guard self-heal.
  - **The surface form is the better dictionary query for `-ed`/`-en`/`-er` endings**, and `resolveSenses` prefers it there. Measured, not assumed: "tired" is listed as an adjective meaning עָיֵף (reducing it gives "tire" → לְהַלאוֹת, *to weary someone*), "broken" → שָׁבוּר, "believer" → מַאֲמִין against the lemma's לְהֶאֱמִין. **`-ing` splits both ways** and is decided by the dictionary's own ordering: "willing" → מוּכָן, "boring" → מְשַׁעֲמֵם and "exciting" → מְרַגֵשׁ list the *adjective* group first and use the surface, while "running" → רִיצָה and "singing" → שִׁירָה list the gerund *noun* first and use the lemma's verb sense. Reducing "willing" to "will" gave רָצוֹן, the noun *will*.
  - **The dictionary's ranking only overrides the model when the model answered about a different part of speech.** `primary` is the translation model's answer for the bare word and reflects actual usage; the dictionary's order is a separate, sometimes older ranking. Taking the dictionary's first entry unconditionally rendered "tend" as לְטַפֵּל בְּ- (*to care for*) — a real sense, but not the one in "what I tend to do", which the model gets right. `selectSense` therefore keeps `primary` unless another part-of-speech group claims it, which is exactly the "tore" case (`primary` is דִמעָה, the noun). Comparison is nikud-insensitive; the two fields are not always pointed alike.
  - **A content word after a subject pronoun is a verb** (`SUBJECT_PRONOUNS` in `pos.ts`). Without that rule the commonest position a verb occupies in a lyric returned UNKNOWN, which then built a NOUN-shaped family — "tend" was listed with יחיד/רבים instead of its conjugation. Guarded by `isContentWord` and `PREVERBAL_ADVERBS` so "I only want" does not conjugate "only". Possessives live in `DETERMINERS`, so the two sets do not overlap.
  - **The repair script never overwrites a translation that is already a valid sense.** Within one part of speech the dictionary's ranking is not evidence: "lay" lemmatises to "lie", whose verb senses are [לְשַׁקֵר, לִשְׁכַּב, …] — *to tell an untruth* first — so taking the top entry would replace a correct לשכב with לשקר. The check is scoped to the inferred part of speech, because unscoped it would also preserve the NOUN sense דִמעָה stored against the verb "tore". Comparison strips nikud and parenthetical transliterations, since stored values span several generations of the translation code.
  - **An absent dictionary block means the lemma is not a word**, and `resolveSenses` falls back to the surface form on that signal. Translating a non-word does not fail — Google renders it phonetically, so "tir" came back as "טיר": real Hebrew letters spelling an English fragment, which sails straight past `looksUntranslated`. The dictionary is the only cheap oracle for "is this even a word".
  - `bestGuessStem` restores a silent `e` that `-ed` swallowed ("tired" → "tir" → **"tire"**), limited to short CVC-shaped stems so complete ones are untouched ("conquered" → "conquer") and to consonants that actually take a silent e (so "fixed" does not become "fixe").
  - `lemmatizeAdjective` now only reduces `-er`/`-est` when a **known word** remains — the posture `deriveRoot` already documented ("otherwise 'water' would lose its '-er'"). A genuine comparative whose stem is missing from the lexicon stays whole: less reduction, never a non-word.
  - `quizGenerator` refuses to build a translation question on a word whose stored translation carries no Hebrew, because rows saved before the guard are still in people's banks. [backend/scripts/repairWordBank.ts](backend/scripts/repairWordBank.ts) re-resolves those rows (dry run by default, `--apply` to write); it re-enriches from the **surface** form, never the suspect stored lemma, and never touches SRS columns.
  - MyMemory signals quota exhaustion with **HTTP 200** and the warning text inside `translatedText`. `parseMyMemoryResponse` rejects that envelope — without the check, an English warning string gets cached as if it were the Hebrew translation. Any new provider needs the same "is this actually a translation?" gate before its result is cached.
  - Only a `TranslationError` advances the chain; anything else propagates, so a bug in our own parsing isn't hidden by the fallback. When all providers fail the 502 body lists each reason — that string is the diagnostic, don't flatten it.
  - Optional `MYMEMORY_EMAIL` raises MyMemory's anonymous quota (its `de` param).
...
- **The translation card follows the tapped word; it does not close on scroll.** The player auto-scrolls the active line into view, so a listener that dismissed on scroll deleted the translation a second or two after it appeared — as soon as playback crossed a line. `TranslationPopover` takes the tapped **element** (not a snapshot rect) and re-reads its position on every scroll, and dismissal is an outside `pointerdown` rather than a full-screen click catcher, since a card that now outlives playback would otherwise leave an invisible overlay blocking the lyrics. `pointerdown` rather than `click` so tapping another word closes this card before that word's own click opens its one.
