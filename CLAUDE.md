...
- **Translation walks a provider chain**, it is not one API. [backend/src/services/translation/providers.ts](backend/src/services/translation/providers.ts) tries Google `gtx` → Lingva → MyMemory; [backend/src/services/translationService.ts](backend/src/services/translationService.ts) is the cache shell around it (`Translation` table, unique on `source+target`, keyed on the lower-cased/whitespace-collapsed source). `GET /api/translate?text=` serves word taps and per-line translation.
  - The chain exists because **every free key-less provider fails on a shared cloud IP**, each differently: `gtx` 429s after a handful of calls per IP, MyMemory has a per-IP daily character quota, Lingva is a community instance that can be down. One provider alone was the cause of "Translation failed" in production while working fine from a laptop. Keep at least two live paths.
