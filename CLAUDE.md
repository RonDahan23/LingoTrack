...
- **Translation walks a provider chain**, it is not one API. [backend/src/services/translation/providers.ts](backend/src/services/translation/providers.ts) tries Google mobile (`translate.google.com/m`, scraped HTML) → Google `gtx` (JSON) → MyMemory; [backend/src/services/translationService.ts](backend/src/services/translationService.ts) is the cache shell around it (`Translation` table, unique on `source+target`, keyed on the lower-cased/whitespace-collapsed source). `GET /api/translate?text=` serves word taps and per-line translation.
  ...
  - **Ordering is by availability, not output quality** (the two Google paths return identical text). `translate.google.com/m` leads because it kept answering from an address already 429'd by `gtx`. Its cost is that it is scraped — `parseGoogleMobileResponse` returns `''` on a markup change so the chain moves on instead of throwing.
